...
4. Use **one replica**, with no cron schedule or public domain. Railway builds the Dockerfile and runs the worker continuously. Disable sleep mode if enabled.
...
The default local file is `ohlcv.sqlite3`. For PostgreSQL set `DATABASE_URL` to its connection URL and install `requirements.txt`. For a large historical first run use `INITIAL_LOOKBACK_MINUTES=10080` (one week). Backfill occurs in requests of up to 1,000 closed candles with a short pause between pages.
...
This service retrieves market data only; it never places trades.
